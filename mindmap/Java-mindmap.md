...
将c转换为数组保存到内部数组对象中，若c中值不为Object[] 则， Array.copyOf为Object[]
	...
	- ArrayList 默认大小为10
其后，默认增长数组(当前数据量)的1/2大小(即扩展到原大小的1.5倍),
...
在物理存储上，每个partition对应一个物理目录，文件夹命名为[topicname]-[partition][序号]，一个topic可以有无数个partition.
...
partition leader与follower的信息受Zookeeper控制，一旦partition leader所在的broker节点宕机，zookeeper从冲其他的broker的partition follower上选择follower变为parition leader。
